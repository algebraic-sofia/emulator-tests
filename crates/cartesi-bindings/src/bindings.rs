...
impl std::fmt::Debug for cm_ram_config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("cm_ram_config")
            .field("length", &self.length)
            .field("image_filename", unsafe {
                &CStr::from_ptr(self.image_filename).to_str().unwrap()
            })
            .finish()
    }
}
...
impl std::fmt::Debug for cm_dtb_config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("cm_dtb_config")
            .field("bootargs", unsafe{  &CStr::from_ptr(self.bootargs).to_str().unwrap() })
            .field("init", unsafe{  &CStr::from_ptr(self.init).to_str().unwrap() })
            .field("entrypoint", unsafe{  &CStr::from_ptr(self.entrypoint).to_str().unwrap() })
            .field("image_filename", unsafe{  &CStr::from_ptr(self.image_filename).to_str().unwrap() })
            .finish()
    }
}
#[test]
...
impl std::fmt::Debug for cm_memory_range_config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("cm_memory_range_config")
            .field("start", &self.start)
            .field("length", &self.length)
            .field("shared", &self.shared)
            .field("image_filename", &if self.image_filename.is_null() {
                "null"
            } else {
               unsafe { CStr::from_ptr(self.image_filename).to_str().unwrap() }
            })
            .finish()
    }
}
...
impl std::fmt::Debug for cm_memory_range_config_array {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let t = unsafe { std::slice::from_raw_parts(self.entry, self.count) };

        f.debug_struct("cm_memory_range_config_array")
            .field("entry", &t)
...
impl std::fmt::Debug for cm_tlb_config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("cm_tlb_config")
            .field("image_filename", unsafe {
                &CStr::from_ptr(self.image_filename).to_str().unwrap()
            })
            .finish()
